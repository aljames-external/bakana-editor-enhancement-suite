...
/**
 * Unified Logger and UI notification dispatcher for Bakana's Editor Enhancement Suite.
...
export class Logger {
    ...
    /**
     * Internal helper to create a styled console group.
     * @param {boolean|null} forceCollapse Explicit collapse override
    ...
    debug(message, ...args) {
        if (this.getVerbosityLevel() >= VERBOSITY_LEVELS.debug) {
            this._ensureGroupsStarted();
            const timestamp = typeof game !== "undefined" ? game?.time?.serverTime ?? "Unknown" : "Unknown";
     ...
     */
    _flushQueues() {
        const uiNotifications = typeof ui !== "undefined" ? ui?.notifications : null;
        if (!uiNotifications) {
